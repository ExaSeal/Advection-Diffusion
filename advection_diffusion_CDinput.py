# -*- coding: utf-8 -*-
"""
Created on Mon Dec  2 20:15:41 2024
...
def matrix_BTCS_dif_periodic(phi, D):
    """
...
def matrix_BTCS_adv_periodic(phi, C):
    """
    Constructs the solution matrix to act upon
...
def BTCS_parallel_split(phi, C, D, nt):
    M_a = matrix_BTCS_adv_periodic(phi, C)
    M_d = matrix_BTCS_dif_periodic(phi, D)
...
def BTCS_Adv_Dif_Periodic(phi, C, D, nt):
    """
...
def BTCS_Adv1_Dif2_Periodic(phi, C, D, nt):
    M_a = matrix_BTCS_adv_periodic(phi, C)
    M_d = matrix_BTCS_dif_periodic(phi, D)
...
def BTCS_Adv2_Dif1_Periodic(phi, C, D, nt):
    M_a = matrix_BTCS_adv_periodic(phi, C)
    M_d = matrix_BTCS_dif_periodic(phi, D)
...
def FTBSCS_Adv_Dif_periodic(phi, C, D, nt):
    nx = len(phi)
    phi_new = np.zeros(nx)
...
def FTBSCS_Adv1_Dif2_periodic(phi, C, D, nt):
    nx = len(phi)
    phi_a = np.zeros(nx)
    phi_ad = np.zeros(nx)
...
def FTBSCS_Adv2_Dif1_periodic(phi, C, D, nt):
    nx = len(phi)
    phi_d = np.zeros(nx)
    phi_da = np.zeros(nx)
...
def FTCSCS_Adv_Dif_periodic(phi, C, D, nt):
    nx = len(phi)
    phi_new = np.zeros(nx)
...
def FTCSCS_Adv1_Dif2_periodic(phi, C, D, nt):
    nx = len(phi)
    phi_a = np.zeros(nx)
    phi_ad = np.zeros(nx)
...
def FTCSCS_Adv2_Dif1_periodic(phi, C, D, nt):
    nx = len(phi)
    phi_d = np.zeros(nx)
    phi_da = np.zeros(nx)
...
def mixed_ADA_DAD(phi, C, D, nt):
    eta = np.sin(2 * np.pi / (1 + (C / D)))
    phi = eta * ADA(phi, C, D, nt) + (1 - eta) * DAD(phi, C, D, nt)
    return phi
